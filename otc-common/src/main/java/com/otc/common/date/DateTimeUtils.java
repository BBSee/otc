...
public final class DateTimeUtils {
  ...
  public static final String REG_exP_DATE = "^((" +
      ...
      +
      "([0-9]{2})(0[48]|[2468][048]|[13579][26])|((0[48]|[2468][048]|[3579][26])00))-02-29))\\s+" +
  ...
  public static int compareDaysWithToday(Date date) {
    ...
    today = new DateTime(today.getYear(), today.getMonthOfYear(), today.getDayOfMonth(), 0, 0, 0,
        0);
    DateTime compareDay = new DateTime(date);
    compareDay = new DateTime(compareDay.getYear(), compareDay.getMonthOfYear(),
        compareDay.getDayOfMonth(), 0,
        0, 0, 0);
  ...
  /**
   * 自动解析多种格式的时间字符串为时间对象<br> 支持格式为：yyyy-MM-dd HH:mm:ss 支持多种分隔符，以及多种日期精度。 如yyyy年MM月。 HH时mm分ss秒
   *
  ...
  private static String formatDateSplit(String md) {
    if (StringUtils.isInt(md)) {
      if (StringUtils.toInt(md) < 10) {
        return "0".concat(String.valueOf(StringUtils.toInt(md)));
      }
    }
    return md;
  }
   ...
   * @return 格式正确返回分割后的时间对象数组 格式错误返回null <br> 指定了格式符为. 分隔符为- 返回值为 时间长度为2的Date类型数组<br> 时间转换的方式详见
